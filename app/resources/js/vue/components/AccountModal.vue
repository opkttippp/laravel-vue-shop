    ...
    <modal-show>
        ...
        <template v-slot:body>
            ...
            <div v-else-if="errors.length" class="alert alert-danger">
             ...
             <h3 style="margin:auto;"><b>{{this.name}}</b></h3>
            <p style="margin:auto;"> Вы действительно желаете удилить свой аккаунт ? </p>
            <div class="personal-data__footer">
                <button type="button" class="btn button--link"
                        @click="this.$emit('close')"
                > Отмена
                </button>
                <button class="btn btn-danger personal-data__edit"
                        @click="this.$emit('AccountDel')"
                >
                    Удалить
                </button>
            </div>
